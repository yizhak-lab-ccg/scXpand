...
def download_pretrained_model(
    ...
    if model_name is not None and model_url is not None:
        raise ValueError(
            "Cannot specify both model_name and model_url. Use one or the other."
        )
    ...
    if model_name is not None:
        ...
        logger.info(
            f"Downloading registry model '{model_name}' to cache directory: {cache_path}"
        )
    else:
        # Use direct URL
        ...
        logger.info(
            f"Downloading model from URL: {model_url} to cache directory: {cache_path}"
        )
    ...
    try:
        model_path = pooch.retrieve(
            ...
            if download_url.endswith((".tar", ".tar.gz", ".tgz"))
            else pooch.Unzip(),
        )
    except Exception as e:
        raise FileNotFoundError(
            f"Failed to download model from {download_url}: {e}"
        ) from e
