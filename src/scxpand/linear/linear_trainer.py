...
class LinearBatchPredictor:
    ...
    def _get_model_properties(self, model: SGDClassifier) -> tuple[bool, bool]:
        ...
        if model_id not in self._model_properties_cache:
            has_predict_proba = hasattr(model, "predict_proba") and callable(
                model.predict_proba
            )
...
class LinearTrainLogger(TrainLogger):
    ...
    def init_linear_training(self, n_epochs: int, n_batches_per_epoch: int) -> None:
        ...
        logger.info(
            f"Linear training initialized: {n_epochs} epochs, {n_batches_per_epoch} batches per epoch"
        )
    ...
    def log_validation_metrics(
        self, epoch: int, dev_set_metrics: dict, score_metric: str
    ) -> None:
...
class TrainingSession:
    ...
    def check_early_stopping(self, current_score: float, epoch: int) -> bool:
        ...
        if is_new_best:
            ...
            logger.info(
                f"No improvement for {self.patience_counter} evaluation intervals"
            )
...
class LinearTrainer:
    ...
    def prepare_data_and_model(
        ...
        logger.info(
            f"Running SGDClassifier with {self.prm.model_type} and parameters: {self.prm}"
        )
        ...
        train_dataset, train_dataloader, dev_dataset, dev_dataloader = (
            self._prepare_datasets_and_dataloaders(
    ...
    def train_epoch(
        ...
        for batch_idx, batch in enumerate(train_dataloader):
            ...
            # Log progress
            if (batch_idx + 1) % self.prm.train_log_interval == 0 or (
                batch_idx + 1
            ) == n_batches:
    ...
    def evaluate_model(
        ...
        predictor = LinearBatchPredictor(
            dataset=eval_dataset, dataloader=eval_dataloader
        )
        ...
        train_logger.log_validation_metrics(
            epoch=epoch, dev_set_metrics=dev_set_metrics, score_metric=score_metric
        )
    ...
    def run_training(
        ...
        model, train_dataset, train_dataloader, dev_dataset, dev_dataloader = (
        ...
        for i_epoch in range(self.prm.n_epochs):
            ...
            if (i_epoch + 1) % self.prm.eval_interval == 0:
                ...
                logger.info(
                    f"Epoch {i_epoch + 1}: {score_metric} = {current_score:.4f}"
                )
                ...
                if session.check_early_stopping(
                    current_score=current_score, epoch=i_epoch
                ):
                    break
