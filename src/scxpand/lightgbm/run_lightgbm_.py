...
def _prepare_data_for_lightgbm_inference(
    ...
    if adata is not None:
        # In-memory data path
        logger.info(
            f"Input data has {adata.n_vars} genes, model expects {data_format.n_genes} genes"
        )
        source_adata = adata
    else:
        ...
        logger.info(
            f"Input file has {adata_file.n_vars} genes, model expects {data_format.n_genes} genes"
        )
    ...
    adata_reordered = data_format.prepare_adata_for_training(
        source_adata, reorder_genes=True
    )
...
def run_lightgbm_training(
    ...
    X_train = load_and_preprocess_data_numpy(
        data_path=data_path, data_format=data_format, row_indices=row_inds_train
    )

    # Apply preprocessing to get validation data
    X_dev = load_and_preprocess_data_numpy(
        data_path=data_path, data_format=data_format, row_indices=row_inds_dev
    )
...
def run_lightgbm_inference(
    ...
    logger.info(
        f"Scikit-learn inference complete. Predictions shape: {predictions.shape}"
    )
    return predictions
