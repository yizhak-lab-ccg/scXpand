...
def report_optuna_trial_result(
    trial: optuna.Trial | None, results: dict, score_metric: str
) -> None:
    ...
    if score_metric not in results_flat:
        logger.warning(
            f"Score metric {score_metric} not found in results: {list(results_flat.keys())}"
        )
        return
...
def _log_trial_score_report(
    trial: optuna.Trial, score_metric: str, formatted_score: str
) -> None:
    """Log the trial score reporting."""
    message = f"Reporting score {score_metric}={formatted_score} to Optuna (trial {trial.number})"
    logger.info(message)
    print(
        f"[Optuna] Reporting score {score_metric}={formatted_score} for trial {trial.number}"
    )


def _handle_trial_pruning(
    trial: optuna.Trial, score_metric: str, formatted_score: str
) -> None:
...
def log_trial_progress(trial: optuna.Trial, study_dir: Path) -> None:
    ...
    formatted_value = (
        format_float(trial_value) if isinstance(trial_value, float) else trial_value
    )
...
def cleanup_incomplete_trials(study: optuna.Study, study_dir: Path) -> int:
    ...
    if cleaned_count > 0:
        logger.info(
            f"Cleaned up {cleaned_count} incomplete RUNNING trial directory(ies)"
        )
...
def _cleanup_single_trial_directory(
    study_dir: Path, trial_number: int, trial_dir: Path
) -> bool:
    ...
    try:
        ...
        logger.warning(
            f"Cleaning up incomplete RUNNING trial directory {trial_number} (age: {age}, no results found)"
        )
...
def _handle_trial_completion(trial: optuna.Trial, study_dir: Path) -> None:
    ...
    if state == optuna.trial.TrialState.PRUNED:
        ...
        logger.info(
            f"Trial {trial_number} was pruned at step {trial.last_step} with value {formatted_value}"
        )
        print(
            f"[PRUNED] Trial {trial_number} at step {trial.last_step} with value {formatted_value}"
        )
