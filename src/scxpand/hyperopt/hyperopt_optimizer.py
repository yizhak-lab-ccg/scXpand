...
class HyperparameterOptimizer:
    ...
    def _handle_existing_study(self) -> None:
        ...
        if not self.resume and self.study_dir.exists():
            ...
            if db_file.exists():
                ...
                trial_dirs = [
                    d
                    for d in self.study_dir.iterdir()
            ...
            if db_file.exists():
                ...
                logger.warning(
                    f"Found existing study database with no trials at {self.study_dir}. Continuing..."
                )
    ...
    def _execute_trial(
        self, trial: optuna.Trial, params: object, save_dir: Path
    ) -> float:
        ...
        try:
            ...
            filtered_args = {
                k: v for k, v in runner_args.items() if k in sig.parameters
            }
        except Exception as e:
    ...
    def _process_trial_result(self, trial: optuna.Trial, result: dict) -> float:
        ...
        score = get_score_from_nested_dict(
            nested_metrics_dict=result, metric_name=self.score_metric
        )

        if score is not None and isinstance(score, float) and math.isnan(score):
            logger.warning(
                f"Trial {trial.number} returned NaN score, marking as failed."
            )
    ...
    def _handle_trial_exception(
        self, trial: optuna.Trial, exception: Exception
    ) -> float:
    ...
    def _resume_existing_trials(self, study: optuna.Study) -> int:
        ...
        for trial in resumable_trials:
            ...
            if trial_dir.exists() and _has_results_indicators(trial_dir):
                logger.info(
                    f"Resuming {trial.state.name} trial {trial.number} from checkpoints"
                )
                ...
                try:
                    ...
                    if score != FAILED_TRIAL_SCORE:
                        logger.info(
                            f"Successfully resumed trial {trial.number} with score {score:.4f}"
                        )
                        resumed_count += 1
    ...
    def run_optimization(self, n_trials: int = 100) -> optuna.Study:
        ...
        if self.resume:
            ...
            if resumed_trials > 0:
                logger.info(
                    f"Resumed {resumed_trials} existing trial(s) from checkpoints"
                )
        ...
        save_study_info(
            study=study, study_dir=self.study_dir, score_metric=self.score_metric
        )
    ...
    def print_results(self, study: optuna.Study | None = None) -> None:
        ...
        try:
            ...
            print(
                f"See {self.study_dir}/{STUDY_INFO_FILE} and {self.study_dir}/trials.log for details."
            )
        except ValueError:
