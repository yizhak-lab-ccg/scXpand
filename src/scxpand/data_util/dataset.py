...
def compute_soft_labels(
    obs_df: pd.DataFrame, dataset_params: DataAugmentParams
) -> np.ndarray:
...
class CellsDataset(Dataset):
    def __init__(
        ...
        self._adata = adata
        self.dataset_params = (
            dataset_params if dataset_params is not None else DataAugmentParams()
        )
        ...
        self._worker_seed_base = hash(
            (id(self), self.dataset_params.mask_rate, self.dataset_params.noise_std)
        )
    ...
    def _preprocess_data(self):
        ...
        if self.is_train:
            ...
            if EXPANSION_COL not in available_columns:
                logger.warning(
                    f"Training mode but '{EXPANSION_COL}' column not found. Labels will not be computed."
                )
            # Start with required columns that exist in the data
            needed_columns = [
                col
                for col in [EXPANSION_COL, CLONE_ID_SIZE_COL, MEDIAN_CLONE_SIZE_COL]
                if col in available_columns
            ]
            # Add optional columns if available
            optional_cols = [
                col
                for col in [TISSUE_TYPE_COL, IMPUTED_LABELS_COL]
                if col in available_columns
            ]
        ...
        if needed_columns:
            self.obs_df = raw_adata.obs.iloc[self.row_inds][needed_columns].reset_index(
                drop=True
            )
        else:
        ...
        if (
            self.is_train
            and CLONE_ID_SIZE_COL in self.obs_df.columns
            and MEDIAN_CLONE_SIZE_COL in self.obs_df.columns
        ):
            self.y_soft = torch.FloatTensor(
                compute_soft_labels(
                    obs_df=self.obs_df, dataset_params=self.dataset_params
                )
            )
        else:
            self.y_soft = None
    ...
    def transform_batch_data(
        self, X_raw: torch.Tensor, in_place: bool = True
    ) -> torch.Tensor:
        ...
        return preprocess_expression_data(
            X=X_tensor, data_format=self.data_format, eps=DEFAULT_EPS
        )
    ...
    def _apply_gene_transformation_tensor(self, X_raw: torch.Tensor) -> torch.Tensor:
        ...
        X_transformed = torch.zeros(
            (batch_size, self.n_genes), dtype=dtype, device=device
        )
        ...
        valid_source_indices = torch.from_numpy(self.gene_indices[valid_mask]).to(
            device
        )
...
def encode_categorical_features_batch(
    ...
    for cat_type in categorical_features_types:
        ...
        valid_indices = np.array(
            [mapping.get(val, -1) for val in values], dtype=np.int32
        )
...
def get_dataloader_kwargs(
    num_workers: int, dataset: "CellsDataset"
) -> dict[str, object]:
...
def compute_categorical_targets_from_batch_obs(
    ...
    for cat_type in dataset.aux_categorical_types:
        if (
            cat_type not in dataset.aux_categorical_mappings
            or cat_type not in batch_obs
        ):
            continue
...
def cells_collate_fn(
    batch_indices: list[int], dataset: "CellsDataset"
) -> dict[str, torch.Tensor]:
    ...
    if dataset.is_train:
        generator = dataset._get_worker_generator()
        X = apply_post_normalization_augmentations(
            X, dataset.dataset_params.noise_std, generator=generator
        )
