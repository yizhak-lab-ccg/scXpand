...
def get_patient_identifiers(obs_df: pd.DataFrame) -> pd.Series:
    ...
    invalid_studies = study_strings[
        study_strings.str.contains(PATIENT_ID_SEPARATOR, na=False)
    ]
    ...
    invalid_patients = patient_strings[
        patient_strings.str.contains(PATIENT_ID_SEPARATOR, na=False)
    ]
...
def calculate_and_log_cancer_distribution(
    ...
    """
    train_cancers = [
        cancer_types_per_patient[uniq_patient_ids.index(pid)]
        for pid in train_patient_ids
    ]
    dev_cancers = [
        cancer_types_per_patient[uniq_patient_ids.index(pid)] for pid in dev_patient_ids
    ]

    train_cancer_distribution = (
        pd.Series(train_cancers).value_counts(normalize=True) * 100
    )
    ...
    cancer_table = metrics_dict_to_table(
        cancer_dist_data, title="Cancer Type Distribution (%)", precision=2
    )
    logger.info(cancer_table)


def calculate_and_log_category_distributions(
    train_obs_df: pd.DataFrame, dev_obs_df: pd.DataFrame
) -> None:
    ...
    for category_type in ["imputed_labels", "tissue_type"]:
        train_distribution = (
            train_obs_df[category_type].value_counts(normalize=True) * 100
        )
        ...
        category_table = metrics_dict_to_table(
            category_dist_data,
            title=f"{category_type.replace('_', ' ').title()} Distribution (%)",
            precision=2,
        )
...
def save_patient_ids(
    save_path: Path, train_patient_ids: list[str], dev_patient_ids: list[str]
) -> None:
    ...
    """
    pd.Series(train_patient_ids).to_csv(
        save_path / "train_patient_ids.csv", index=False
    )
...
def split_data(
    ...
    cancer_types_per_patient = validate_patient_cancer_types(
        uniq_patient_ids, patient_identifiers, obs_df
    )
