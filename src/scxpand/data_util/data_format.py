...
class DataFormat(BaseModel):
    ...
    def create_data_format(
        ...
        if self.use_zscore_norm:
            logger.info(
                "Computing empirical mean and std per gene on the normalized training set"
            )
            ...
            logger.info(
                "Skipping gene statistics computation (z-score normalization disabled)"
            )
        ...
        if self.aux_categorical_types:
            ...
            for feature_type in self.aux_categorical_types:
                if feature_type in train_obs.columns:
                    ...
                    logger.warning(
                        f"Categorical target type {feature_type} not in the AnnData object"
                    )
            logger.info(
                f"Auxiliary target categorical mappings: {self.aux_categorical_mappings}"
            )
    ...
    def save(self, save_path: Path) -> None:
        ...
        np.savez_compressed(
            npz_path, genes_mu=self.genes_mu, genes_sigma=self.genes_sigma
        )
        logger.info(f"DataFormat saved to {save_path}")

    def _analyze_gene_differences(
        self, adata: AnnData
    ...
    def _reorder_genes_only(
        self, adata: AnnData, curr_gene_names: list[str]
    ) -> AnnData:
    ...
    def convert_genes_expression_matrix(self, adata: AnnData) -> AnnData:
        ...
        """
        curr_gene_names, missing_genes, superfluous_genes = (
            self._analyze_gene_differences(adata)
        )

        # If no changes needed, return
        if (
            not missing_genes
            and not superfluous_genes
            and curr_gene_names == self.gene_names
        ):
            logger.info("No gene changes needed, returning the original data")
            return adata
        else:
            logger.info(
                "Gene order/content differences detected - reordering genes to match data format"
            )
        ...
        return self._handle_gene_differences(
            adata, curr_gene_names, missing_genes, superfluous_genes
        )

    def _handle_gene_differences(
        self,
        adata: AnnData,
        curr_gene_names: list[str],
        missing_genes: set[str],
        superfluous_genes: set[str],
    ) -> AnnData:
        ...
        if len(superfluous_genes) > 0:
            logger.info(
                f"Identified {len(superfluous_genes)} superfluous genes for removal"
            )
            del adata
        ...
        return self._reorder_final_genes(
            common_adata, curr_gene_names, missing_genes, superfluous_genes
        )

    def _add_missing_genes(
        self, common_adata: AnnData, missing_genes: set[str], n_obs: int
    ) -> AnnData:
    ...
    def prepare_adata_for_training(
        self, adata: AnnData, *, reorder_genes: bool = False
    ) -> AnnData:
            ...
            for key, mapping in mappings.items():
                if len(mapping) <= 3:
                    ...
                    formatted_mappings.append(
                        f"'{key}': {dict(sample_items)} (total: {len(mapping)} categories)"
                    )
