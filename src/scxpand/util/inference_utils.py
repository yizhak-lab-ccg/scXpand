...
def load_model(
    model_type: ModelType | str, model_path: Path, device: torch.device | str
    ...
    try:
        ...
        raise ValueError(
            f"Unsupported model_type for loading: '{model_type}'"
        ) from None
    ...
    if model_type_enum == ModelType.AUTOENCODER:
        ...
        raise ValueError(
            f"Unsupported model_type for loading: '{model_type_enum.value}'"
        )
...
def run_model_inference(
    ...
    try:
        ...
        # Re-raise with the expected error message format for backward compatibility
        raise ValueError(
            f"Unsupported model_type for inference: '{model_type}'"
        ) from None
...
def setup_inference_environment(
    ...
    logger.info(
        f"Inference environment ready: {model_type_enum.value} model on {device}"
    )
