...
def load_model_state_dict(
    ...
    try:
        ...
        raise ModelLoadingError(
            f"Failed to load {model_name} model from {best_ckpt_path}: {e}"
        ) from e


def load_model_parameters(
    model_dir: Path, param_class: type, param_file: str = PARAMETERS_FILE
) -> Any:
    ...
    try:
        ...
        if not isinstance(param_dict, dict):
            raise ValueError(
                f"Parameter file must contain a JSON object: {params_path}"
            )
        ...
        raise json.JSONDecodeError(
            f"Invalid JSON in parameter file {params_path}: {e.msg}", e.doc, e.pos
        ) from e
    except TypeError as e:
        raise TypeError(
            f"Failed to instantiate {param_class.__name__} from {params_path}: {e}"
        ) from e
