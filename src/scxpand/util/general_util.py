...
def compute_row_sums(X) -> np.ndarray:
    ...
    if hasattr(X, "sum"):
        if isinstance(X, torch.Tensor):
            ...
            return (
                np.asarray(row_sums).flatten()
                if hasattr(row_sums, "flatten")
                else row_sums
            )
    else:
        raise TypeError(
...
def compute_scaling_factors(
    row_sums: np.ndarray, target_sum: float, dtype: type = np.float32
) -> np.ndarray:
...
def get_new_version_path(save_path: Path | str, start_index: int = 1) -> Path:
    ...
    max_version = (
        np.max(existing_versions) if len(existing_versions) > 0 else start_index - 1
    )
    ...
    try:
        ...
        remote_url = (
            subprocess.check_output(
                ["git", "config", "--get", "remote.origin.url"], timeout=5
            )
            .strip()
            .decode("utf-8")
        )
...
def compute_false_positive_rate(
    label: np.ndarray, prob_out: np.ndarray, threshold: float = 0.5
) -> float:
...
def compute_error_rate(
    label: np.ndarray, y_pred: np.ndarray, threshold: float = 0.5
) -> float:
...
def nested_dict_to_flat_str(
    nested_scalars: dict, omit_keys: list[str] | None = None
) -> str:
    ...
    return ", ".join(
        [
            (
                f"{k}={format_float(v, precision=4)}"
...
def nested_dict_to_multiline_str(
    nested_scalars: dict, indent: int = 0, oneline_last_level: bool = True
) -> str:
    ...
    if oneline_last_level and all(
        isinstance(v, int | float | str) for v in nested_scalars.values()
    ):
        return ", ".join(
            (
                f"{k}={format_float(v, precision=4)}"
        ...
        for key, value in nested_scalars.items():
            if isinstance(value, dict):
                ...
                lines.append(
                    " " * indent + f"{key}: {format_float(value, precision=4)}"
                )
            else:
...
def log_nested_metrics(
    ...
    if score_metric:
        ...
        if score_metric in flat_metrics:
            ...
            logger_func(
                f"{prefix}Warning: Score metric '{score_metric}' not found in metrics"
            )
    ...
    if metrics:
        if use_table_format:
            ...
            full_msg = f"{prefix}{group.capitalize()} metrics details{epoch_str}:\n{nested_dict_to_multiline_str(metrics)}"
    ...
    if kwargs:
        ...
        for field_name, new_value in kwargs.items():
            ...
            if old_value != new_value:
                if logger:
                    logger.info(
                        f"Parameter override: {field_name} = {new_value} (was {old_value})"
                    )
...
def metrics_dict_to_table(
    metrics: dict, title: str = "Metrics", precision: int = 4
) -> str:
    ...
    if overall_metrics:
        ...
        overall_df = pd.DataFrame(
            list(sorted_overall.items()), columns=["Metric", "Value"]
        )
        # Format float values
        ...
        table_str = tabulate(
            overall_df, headers="keys", tablefmt="simple", showindex=False
        )
    ...
    if category_metrics:
        ...
        table_str = tabulate(
            category_df, headers="keys", tablefmt="simple", showindex=True
        )
        lines.append(table_str)
        lines.append("")  # Add newline after table
...
def metrics_dict_to_dataframes(
    metrics: dict, precision: int = 4
...
def log_inference_progress(
    ...
    if (current_iteration + 1) % log_interval == 0:
        ...
        logger_instance.info(
            f"Inference: {progress:.1%} | Time: {elapsed:.1f}s / ~{estimated_total:.1f}s"
        )
