...
def plot_roc_curve(
    ...
    if labels.sum() == 0 or labels.sum() == len(labels):
        logger.info(
            f"No positive labels found for {plot_name}. Returning NaN and skipping plot."
        )
        return np.nan
...
def plot_roc_curves_per_strata(
    ...
    if not available_cols:
        logger.warning(
            f"None of the stratification columns {strata_columns} are available. Skipping per-strata plots."
        )
        return {}

    if len(available_cols) < len(strata_columns):
        logger.warning(
            f"Some stratification columns missing: {set(strata_columns) - set(available_cols)}"
        )
    ...
    for stratum_name in unique_strata:
        ...
        if len(np.unique(labels_strata)) >= 2 and not (
            np.isnan(preds_strata).any() or np.isinf(preds_strata).any()
        ):
