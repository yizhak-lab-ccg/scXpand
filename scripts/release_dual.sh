...
# scXpand Dual Package Release Script
# Publishes both standard (scxpand - CPU/MPS) and CUDA-enabled (scxpand-cuda) versions
# Usage: ./scripts/release_dual.sh [--dry-run] [--patch|--minor|--major]
...
check_prerequisites() {
    ...
    # Check if pyproject file and build script exist
    if [ ! -f "pyproject.toml" ]; then
        print_error "pyproject.toml not found"
        exit 1
    fi

    if [ ! -f "scripts/build_packages.sh" ]; then
        print_error "scripts/build_packages.sh not found"
        exit 1
    fi
...
build_and_test_packages() {
    ...
    if [ "$DRY_RUN" = true ]; then
        ./scripts/build_packages.sh --dry-run --clean
    else
...
show_summary() {
    ...
    if [ "$DRY_RUN" = true ]; then
        ...
        print_status "To perform the actual release, run: ./scripts/release_dual.sh --$VERSION_TYPE"
    else
...
main() {
    echo "=========================================="
    if [ "$DRY_RUN" = true ]; then
        echo "  scXpand Dual Package Release - DRY RUN"
    else
        echo "  scXpand Dual Package Release Script"
    fi
    echo "=========================================="
    echo
